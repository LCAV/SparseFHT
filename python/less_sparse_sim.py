...
n = 22   # Transform size 2**n
b = 17   # Number of buckets 2**b
C = 4    # Oversampling by 4

# average number of non-zero coeffs per bin
#beta = np.arange(1, 4.25, 0.25)
beta = np.r_[np.linspace(1., 2., 4, endpoint=False),
             np.linspace(2., 2.5, 4, endpoint=False),
             np.linspace(2.5, 3.5, 20, endpoint=False),
             np.linspace(3.5, 4., 5)]
...
if __name__ == '__main__':
    ...
    if data_filename is None:
        ...
        data_filename = 'data/{}_less_sparse_sim.npz'.format(date)
